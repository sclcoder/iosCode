...
// https://abson.github.io/2018/04/02/rac-source-analyze/
static Class RACSwizzleClass(NSObject *self) {
    
	Class statedClass = self.class; /// 由于KVO重写了class 方法，所以不能准确的找到类
	Class baseClass = object_getClass(self); /// 方法可以准确的找到 isa 指针.
    /**
        object.class 与 object_getClass(object) 进行判断 来防止KVO导致的AOP无效.
     */
    
    
    /// 对象会关联一个RAC生成的subclass
	...
	if (statedClass != baseClass) {
        ...
        /// swizzledClasses() 专门用来存储 isa 被修改过的类，这些类不用重新生成为 RAC 的类.
	...
	if (subclass == nil) { /// 生成的subclass继承对象原来的class
	...
	object_setClass(self, subclass); /// 更改对象的class,指向RAC创建的subclass
